...
export async function startConsumer() {
  ...
  await consumer.run({
    eachMessage: async ({ message }: EachMessagePayload) => {
      ...
      try {
        const parsed: BufferMessage = JSON.parse(message.value.toString());
        buffer.push(parsed);

        // If this is the first message in the buffer, start the flush timer
...
async function flushBufferToDb() {
  ...
  try {
    const prismaPayload = toInsert.map((msg) => ({
      conversationId: msg.coversationId,
    ...
    //  Redis unseen counter (only if DB insert succeeded)
    ...
    if(!flushTimer) flushTimer = setTimeout(flushBufferToDb, BATCH_INTERVAL_MS);
  }
}
