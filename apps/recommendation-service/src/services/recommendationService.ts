...
export const recommendProducts = async (
  ...
  interactions.forEach(({ userId, productId }) => {
    if (!(userId in userMap)) {
      userMap[userId] = userCounter++;
    }
    if (!(productId in productMap)) {
      productMap[productId] = productCounter++;
    }
  });
  ...
  // create embedding layer (like lookup tables) to learn the relationship between users and products
  const userEmbedding = tf.layers
    .embedding({
      inputDim: userCounter,
  ...
  // sort and select top 10 recommended products based on predicted scores
