...
export const createPaymentIntent = async (
  ...
  const platformFee = Math.round(customerAmount * 0.01);
...
export const createOrder: RequestHandler = async (
  ...
  try {
    ...
    if (!stripeSignature) {
      return res.status(404).send("Missing the stripe signature!");
    }
      ...
      for (const shop of sellerShops) {
        ...
        await prisma.notifications.create({
          data: {
            ...
            message: `A customer just ordered ${productTitle} from ${shop.name} shop`,
            creatorId: userId,
            recieverId: "admin",
            redirect_link: `https://eshop.com/order/${sessionId}`,
          },
